...
```{r}
especies_traits <- data_read(here("traits/aves_traits_data/aves_traits_obtained/aves_cr_traits_ebird.xlsx")) |>
  clean_names()
```

```{r}
especies_traits$migration <- as.numeric(especies_traits$migration)
```
...
especies_traits <- especies_traits |>
  select(species, beak_length_nares, tarsus_length, tail_length, kipps_distance, hand_wing_index, mass)
```
...
```{r}
sitios_especies_6 <- data_read(here("traits/aves_data/aves_pa_in_polygons_6_df.xlsx"))
```
...
```{r}
raoq <- fundiversity::fd_raoq(especies_traits, sitios_especies_6)
```
